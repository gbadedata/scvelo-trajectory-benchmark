...
def validate_dataset(adata: ad.AnnData) -> None:
    ...
    Checks for spliced/unspliced layers and cell-type labels.
    Note: dpt_pseudotime is computed during preprocessing (Phase 2)
    using sc.tl.dpt() rooted at Ductal cells -- it is not pre-stored
    in the pancreas h5ad from scvelo's repository.
    ...
    if missing_obs:
        raise ValueError(
            ...
            "The pancreas dataset must contain cell-type cluster labels."
        )

    n_cells = adata.n_obs
    n_genes = adata.n_vars
    logger.info(
        "dataset_validated: %d cells x %d genes, layers=%s, cell_types=%s",
        n_cells,
        n_genes,
        list(adata.layers.keys()),
        list(adata.obs["clusters"].unique()),
    )
